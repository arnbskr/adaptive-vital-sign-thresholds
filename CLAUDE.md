...
`data/raw/`, `data/processed/`, and generated RAG artifacts (`data/rag_documents/`, `rag_chunks/`, `rag_index/`) are **gitignored** because they contain MIMIC-IV data, which is access-controlled — never commit derived patient data. `data/chroma_db/` is committed (it's the prebuilt index). The repo is bilingual: code/docs in English, R comments and some commit messages in French.

## Legacy (do not extend)

These scripts predate the semantic pipeline and are unused by `app.py`: the TF-IDF-era `src/prepare_rag_documents.py`, `chunk_documents.py`, `build_rag_index.py`, `retrieve_chunks.py`, `rag_utils.py`; and the standalone `src/rag.py` / `src/generate_rag_answer.py` retrieval demos. `src/bigquery_extract_mimic.py` is still relevant as the upstream extractor but is not part of the runtime app. Prefer the active pipeline above.
